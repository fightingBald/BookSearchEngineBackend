...
def remove_funny_tokens(text):# remove funny tokens
    tokens = text.split()
...
df_metadata = pd.read_csv('../searchEngineProject/gutenberg_metadata.csv')# load metadata

data = {'Author': None, 'Title': None, 'Link': None, 'ID': None, 'Bookshelf': None, 'Text': None}# create empty dataframe


BOOK_OCUNTER = 0
for key, row in df_metadata.iterrows():# iterate over metadata
    BOOK_OCUNTER += 1
    if BOOK_OCUNTER == DATASET_SIZE:# limit dataset size
        break
    ...
    print('Finished ' + str(key + 1) + ' of ' + str(len(df_metadata)))
